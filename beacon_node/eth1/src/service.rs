...
impl Service {
    ...
    ///
    /// Emits logs for debugging and errors.
    pub async fn auto_update(&self, exit: Exit) -> Result<(), ()> {
        ...
        loop {
            ...
            // TODO: Seems inefficient to clone every 7 seconds. Improve this.
            // This future should be Unpin to pass to `exit.until`.
            // However, putting it in an async block makes it !Unpin.
            if futures::poll!(exit.clone()) == Poll::Ready(()) {
                break;
            }
        }
